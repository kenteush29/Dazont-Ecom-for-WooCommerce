...
(function ($) {
	...
	$(document).on('click', '#dze-auto-count', function () {
		...
		$.post(d.ajaxUrl, {
			...
			if (!res.success) { $out.css('color', '#b32d2e').addClass('is-ko').text(d.i18n.error); return; }
			var txt = d.i18n.result.replace('%1$s', res.data.total).replace('%2$s', res.data.applied);
			$out.css('color', '#0a7040').removeClass('is-ko').text(txt);
			autoProducts = res.data.products || [];
			if (autoProducts.length) { $('#dze-auto-list').show(); }
		}).fail(function () { $out.css('color', '#b32d2e').addClass('is-ko').text(d.i18n.error); });
	});
	...
	$(document).on('click', '#dze-hero-make', function () {
		...
		$msg.css('color', '#646970').removeClass('is-ko').text(cfg.i18n.heroMaking);
		$.post(cfg.ajaxUrl, {
			...
			if (!res || !res.success) {
				$msg.css('color', '#b32d2e').addClass('is-ko').text((res && res.data && res.data.message) || cfg.i18n.heroFailed);
				return;
			}
			$cell.find('input[type=hidden]').val(res.data.id);
			$cell.find('.dze-hero-preview').attr('src', res.data.url).show();
			$cell.find('.dze-hero-clear').show();
			$msg.css('color', '#0a7040').removeClass('is-ko').text(cfg.i18n.heroDone);
		}).fail(function () {
			$b.prop('disabled', false);
			$msg.css('color', '#b32d2e').addClass('is-ko').text(cfg.i18n.heroFailed);
		});
	});
	...
	$(document).on('click', '#dze-banner-translate', function () {
		...
		$.post(cfg.ajaxUrl, { action: 'dze_mai_translate', nonce: cfg.maiNonce, title: line })
			...
			.fail(function () {
				$b.prop('disabled', false);
				$st.css('color', '#b32d2e').addClass('is-ko').text(cfg.i18n.trFailed);
			});
	});
	...
	$(document).on('click', '#dze-rule-bulk button[type="submit"]', function (e) {
		if (!$('.dze-rule-cb:checked').length) {
			e.preventDefault();
			$('#dze-gmc-bulk-status').css('color', '#b32d2e').addClass('is-ko').text(
