 ...
 *    pilot or by hand — is never rewritten; a picture chosen is never
 *    replaced; a plan is asked once per promotion, so emails the owner
 *    deleted stay deleted. Stopping the pilot is deleting the rows or
 *    switching the setting off, and both stick.
...
final class DZE_Klaviyo_Auto {
	...
	public static function next_step( array $rule, array $emails, array $auto, array $ctx ): array {
		...
		// The plan, once per promotion. Emails already there — planned, added
		// by hand, or left from before — count as the plan having happened:
		// asking again would put new rows beside what the owner has curated.
		if ( ! $emails && empty( $auto['planned'] ) ) {
	...
	public static function step( string $rule_id ): array {
		...
		try {
			...
			// the pilot existed. They count as the plan having happened, and
			// they are marked so the pilot completes them (writes, drafts,
			// translates) but never SCHEDULES them: see next_step().
			if ( ! $auto && $emails ) {
				$auto = [ 'planned' => 1, 'legacy' => 1 ];
			...
			try {
				...
				if ( 'plan' === $next['do'] ) {
					...
					self::write_auto( $rule_id, [ 'planned' => 1, 'legacy' => 0 ] );
				}
			} catch ( \Throwable $e ) {
	...
	public static function status_line( string $rule_id, array $rule ): string {
		...
		$total = count( $emails );
		if ( 0 === $total ) {
			return __( 'Autopilot: on — the campaign is planned as soon as the event is saved with its dates.', 'dazont-ecom' );
		}
		$written = 0;
		$drafted = 0;
		$i18ned  = 0;
		...
		$line = __( 'Autopilot:', 'dazont-ecom' ) . ' ' . implode( ' · ', $bits );
		if ( 'done' === $next['do'] ) {
