...
PROMPT;
	...
	public function render_panel( int $pid ): void {
		...
		<div class="dze-rev-box" data-post="<?php echo (int) $pid; ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( self::NONCE ) ); ?>">
			...
			<p>
				...
				<?php
				// ONE WAY TO READ, EDIT AND SAVE A PROMPT, everywhere in the
				// plugin: the popup. This panel kept an inline editor of its own
				// behind a lone pencil — a second surface that drifted from the
				// first, and one that sent whatever it happened to be showing as
				// a one-off override, so Generate ran two different instructions
				// depending on whether the box was open. The category panel was
				// mended of exactly this; this one was forgotten.
				if ( class_exists( 'DZE_Prompts' ) ) {
					DZE_Prompts::the_button( 'reviews' );
				}
				?>
	...
	public function ajax_generate(): void {
		...
		// THE SAVED PROMPT, ALWAYS. A one-off override posted from an inline
		// editor is how one button came to run two different instructions.
		try {
