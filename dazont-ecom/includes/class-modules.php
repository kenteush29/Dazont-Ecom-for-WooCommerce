	...
	public static function catalog(): array {
		return [
			...
			'marketing_ai' => [
				...
				'more'  => __( 'Builds a proposed marketing calendar from what the shop says about itself — the description written once under Settings → General → About this shop, which every module of the plugin reads — plus the languages (WPML) and, per language, the countries to consider, ticked one by one before each generation. The catalogue can be sent on top of it, and usually should not be: a calendar is built on commercial moments, not on a list of best sellers. Every suggestion is reviewed by you — accepting turns it into a real scheduled event in Marketing Events; a shortcode renders the final calendar on the front. This module also hosts the shared Settings page (API keys, model choices, monthly spend cap) that the other modules read their configuration from. Everywhere the plugin is about to call a model — the calendar here, the sourcing report, the category writer, the product texts and images — a discreet pencil opens the exact instructions being sent and takes you straight to the field where they are edited, on the tab that owns them.', 'dazont-ecom' ),
			],
			'sourcing' => [
