	...
	public static function catalog(): array {
		return [
			...
			'klaviyo' => [
				...
				'desc'  => __( 'Turns a marketing event into a draft campaign in Klaviyo.', 'dazont-ecom' ),
				'more'  => __( 'A promotion the shop runs and nobody is told about is half a promotion. This takes an event from Marketing Events and builds the email that announces it, in the account that already sends the shop\'s campaigns. It does what you would do by hand, in the same order: your promotion template is CLONED — the original is never touched, exactly as Klaviyo does when you edit a campaign\'s content — the markers that template carries ({{PROMO_TITLE}}, {{PROMO_PERCENT}}, {{PROMO_START}}, {{PROMO_END}}, {{PROMO_URL}}) are replaced by this event\'s own figures wherever they appear, in a heading as in a button or a link, a campaign is created for the audience chosen once in the settings — one campaign for everybody, minus the exclusion you name — and the copy is assigned to it. Every value is on screen and editable before anything is created, so a promotion whose button should point at a category rather than the home page is one field away. It never sends and never schedules: what comes out is a DRAFT, carrying the send date, and the decision to send stays in Klaviyo in front of the campaign. Three things are not asked, because they have one sane answer each: the draft goes out under the sender verified on your Klaviyo account, it is dated the day the promotion opens at nine in each reader\'s own time zone (and that field is in front of you on the campaign itself), and the subject is written in every language. Language is not handled by a second campaign per market: profiles carry the language your account assigned them, and Klaviyo\'s own translator serves each reader in his — except the promotional line itself, the one thing a machine translator writes worse than the shop does, which is taken market by market from the adapted lines the event already carries. The email itself is written on the event\'s own screen, under the promotion it announces: the subject, the preview text and one field for every marker your template carries — a heading, a body, a button label, whatever you named them — with the pieces the event answers by itself shown filled and locked, because an email that disagrees with its own promotion is worse than no email. One button writes the whole thing with Claude under instructions you edit like any other prompt; another shows the email as it will actually look, rendered by KLAVIYO from the very template the campaign will use, so what is on screen cannot drift from what is sent. It is saved by the event\'s own Save button — the page has one form and one button — and the draft is then created from what was written. The private API key is only ever sent to Klaviyo, the account is only ever read behind a click — no page of this plugin, and no page of the shop, calls Klaviyo while somebody waits for it.', 'dazont-ecom' ),
			],
			'sourcing' => [
