	...
	public static function catalog(): array {
		return [
			...
			'link_mesh' => [
				'class' => 'DZE_Link_Mesh',
				'group' => 'catalog',
				'label' => __( 'Internal linking upkeep', 'dazont-ecom' ),
				'desc'  => __( 'Adds the missing internal links to your categories, a few a day.', 'dazont-ecom' ),
				'more'  => __( 'A catalogue is not linked once and for all: categories are added, descriptions are rewritten, a branch grows — and pages nothing points at appear all the time. Finding them by hand means re-reading the whole tree. This module watches for them and fixes them at a human pace. It writes nothing of its own: the pass it runs is the linking-only pass the Category descriptions module already offers on each category — the text is left exactly as it is, only links are added — and the writing itself is handed to the Writing queue, in the background, one at a time, under the same monthly budget cap as everything else. What it decides is which category and when. Which: the category the rest of the shop points at LEAST, counted from the links every other category description actually contains, among those whose description is long enough to weave a link into and whose size still allows one more. Orphans first, always. When: one category per hour at most, a figure per day you set (three by default — a hundred categories in a month), and never the same category twice within a month. A shop whose fifty pages all change on the same afternoon does not look like a shop being looked after. Descriptions are edited on the shop without being reviewed first, so the description each pass replaced is kept: the last ten are listed with what they gained, and one click puts any of them back. Turning it off stops the hourly look; the figures, the log and the undos stay where they are.', 'dazont-ecom' ),
			],
			'reviews' => [
				'class'   => 'DZE_Reviews',
				'group'   => 'product',
