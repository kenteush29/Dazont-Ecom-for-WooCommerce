	...
	public static function catalog(): array {
		return [
			...
			'klaviyo' => [
				...
				'more'  => __( 'A promotion the shop runs and nobody is told about is half a promotion. This builds the email that announces it and files it as a DRAFT campaign in the account that already sends the shop\'s campaigns. The email is built here rather than fetched from a template: the mark at the top, the colours, the footer and the frame belong to the plugin and are set once — so nothing has to be kept in step in two places, and there is no marker discipline to keep up in Klaviyo. What a promotion actually says is written on the event\'s own screen, in named fields: a headline, two or three sentences, the offer in a box of its own, a button and where it goes, an optional full-width image from the media library, and three of the shop\'s best-sellers read from WooCommerce at the price the promotion gives them. One button writes the lot with Claude, from the event\'s figures and the shop\'s description, under instructions you edit like any other prompt; another draws the email exactly as it will be sent — the same function builds both, so the preview cannot drift from the send. It is saved by the event\'s own Save button: the page has one form and one button. Creating the draft then files that HTML as a template in Klaviyo, opens a campaign for the audience chosen once — one campaign for everybody, minus the exclusion you name — assigns the email to it, and stops. It never sends and never schedules: the draft carries its send date (the day the promotion opens, at nine, in each reader\'s own time zone) and the decision stays in Klaviyo. Language follows the account\'s own method: profiles carry the language assigned to them and Klaviyo\'s translator serves each reader in his, except the promotional line itself — the one a machine translator writes worse than the shop does — which is taken market by market from the adapted titles the event already carries. The audience is chosen once, and the screen that chooses it can act on the account rather than send you to it: segments Klaviyo hides from its own listing because they are inactive are offered anyway and marked as such, the chosen one can be switched back ON from here — an inactive segment holds nobody, so excluding it excludes nobody — and the exclusion every promotion wants can be BUILT from here: the buyers of the last N weeks, on this account\'s own order metric rather than a hard-coded id, because announcing a sale to somebody who paid full price three days ago earns a refund request rather than an order. The private API key is only ever sent to Klaviyo, the account is only ever read or written behind a click, and no page of the shop calls it at all.', 'dazont-ecom' ),
			],
			'sourcing' => [
