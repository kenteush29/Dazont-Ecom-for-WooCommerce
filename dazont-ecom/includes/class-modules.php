	...
	public static function catalog(): array {
		return [
			...
			'klaviyo' => [
				...
				'more'  => __( 'A promotion the shop runs and nobody is told about is half a promotion. This writes the email that announces it and files it as a DRAFT campaign in the account that already sends the shop\'s campaigns. The header and the footer are FIXED and not editable — the shop\'s logo at the top, the three promises and the legal lines at the bottom — because a header that changes from one campaign to the next is a shop the reader stops recognising; and the type and the colours are read from the theme, so an email looks like the site it comes from without a single setting to fill. Between the two, one panel with a Code / Preview switch: the email is HTML, and the preview is drawn by the very function that builds what Klaviyo receives, so the two cannot drift apart. One button writes the whole thing — subject, preview line, heading, image, text, button and a row of products — from the event\'s figures and the shop\'s own description, under instructions you edit like any other prompt; the picture is the one the event already carries for its homepage swap, and the products are read from WooCommerce (what actually sold in the last fortnight, restricted to the event\'s categories when it names any) rather than invented. Two buttons insert an image from the media library or that product row at the cursor. It is saved by the event\'s own Save button: the page has one form and one button. Creating the draft files that HTML as a template in Klaviyo, opens a campaign for the audience chosen once — one campaign for everybody, minus the exclusion you name — assigns the email to it, and stops. It never sends and never schedules: the draft carries its send date (the day the promotion opens, at nine, in each reader\'s own time zone) and the decision stays in Klaviyo. Language follows the account\'s own method: profiles carry the language assigned to them and Klaviyo\'s translator serves each reader in his, except the promotional line itself, which is taken market by market from the adapted titles the event already carries. The audience is chosen once, and the screen that chooses it can act on the account rather than send you to it: segments Klaviyo hides from its own listing because they are inactive are offered anyway and marked as such, the chosen one can be switched back ON from here, and the exclusion every promotion wants can be BUILT from here — the buyers of the last N weeks, on this account\'s own order metric. The private API key is only ever sent to Klaviyo, the account is only ever read or written behind a click, and no page of the shop calls it at all.', 'dazont-ecom' ),
			],
			'sourcing' => [
