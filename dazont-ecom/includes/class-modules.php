	...
	public static function catalog(): array {
		return [
			...
			'diagnostic' => [
				...
				'more'  => __( 'The question that comes before every other one: WHERE. A shop of a thousand products has no memory of which of them is short of a paragraph, which has one photograph in its gallery and which never got its second custom block, and a catalogue audited once in a spreadsheet is out of date the week after. This reads the shop and says so — and it is a to-do list, not a machine: nothing here writes, generates, spends or decides, and every line points at the screen that already fixes that one thing. The criteria are not this plugin\'s opinions: they are a LIST YOU EDIT, in the shape the rest of the plugin uses for its prompts and its email types — a name, one menu saying what is looked at (\'Product · description\', \'Product · gallery photographs\', \'Product · custom field\' with its key, \'Category · internal links\', \'Article or page · internal links\'), the rule it has to pass (empty, fewer than N words, fewer than N), a switch, and a cross to remove it. Add your own, change a figure, restore the shipped list in one click. Seven ship, and they are rows like any other rather than code, so \'restore the default\' is putting those seven back and nothing else. Every other criterion is READ FROM YOUR OWN PROMPTS — each prompt already declares what it writes and where, so "Custom bloc text 2: empty on 340 products" is a question the shop can already answer about itself, and the photograph a text block argues about is counted separately from the text. Add a prompt and its line appears; switch it off and the line goes with it; there is no second list to keep in step, and no criterion counts the same product twice. Articles and pages are read by the linking pass\'s own census, so this screen and that one can never disagree about the same article. A criterion you would rather not be counted against is unticked in the settings — nothing is deleted, it simply stops being called work. The reading is done once a night in cron and kept: a screen that counted a thousand products on every load is a screen nobody opens twice, and the count on the menu is read from what was stored. "Read the shop again" does it on demand, once, with a lock so two clicks are one reading. Each line gives its count, what share of the shop that is, and the list behind it — where every entry also says what ELSE that product is short of, so a product needing four things is opened once instead of four times.', 'dazont-ecom' ),
			],
			'automation' => [
