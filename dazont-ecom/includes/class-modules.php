	...
	public static function catalog(): array {
		return [
			...
			'automation' => [
				...
				'more'  => __( 'Adds no ability of its own: everything it launches is a function already on this site, with its own screen, its own prompt and its own button. What it brings is the part nobody can do by hand at scale — looking at the WHOLE catalogue, seeing where each page falls short of what it should be, and working through the list a little at a time. It judges a page on the very figures that page\'s own panel prints: "1116 words, 0 links — target 750 words and 5 links" is read from the same place the panel reads it, so the screen and the robot can never disagree. Two tasks today, each with its own switch, its own figure per day and its own choice between saving straight to the shop or waiting under "to review": internal links on categories, which runs the "Add internal links only" pass on the category the rest of the shop points at LEAST — inbound links counted from what the other descriptions actually contain, orphans first — among those still under their link target; and category descriptions, for a category that has none or one far under the length its branch deserves, held for review by default since a rewrite replaces the whole text, and restricted to categories that have their SEMrush file unless you say otherwise. The work itself is never done here: it is handed to the Writing queue, one job at a time, in the background, under the monthly budget cap. The pace is the point — one item per pass, spread over the day (three a day is one every eight hours, not three before breakfast), and never the same category twice within a month, unless the pass changed nothing, in which case it comes back in three days rather than being locked out. What was saved straight to the shop keeps the text it replaced: the last dozen passes are listed with what they changed, and one click puts any of them back. Adding a task later is adding a row to its list: a label, the module that owns the work, and how a page short of something is recognised.', 'dazont-ecom' ),
			],
			'reviews' => [
				'class'   => 'DZE_Reviews',
				'group'   => 'product',
