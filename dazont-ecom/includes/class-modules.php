	...
	public static function catalog(): array {
		return [
			...
			'translate' => [
				...
				'more'  => __( 'A translation desk of its own, under Dazont Ecom → WPML Translations, built the way WPML\'s own Translation Management is: a dashboard saying where the site stands language by language, a batch you choose and send, and a waiting list where every translation is read before it lands. WHAT MAY BE TRANSLATED IS WPML\'S ANSWER, never ours: the post types and the taxonomies are read from WPML\'s own settings — products, articles, pages, product categories and tags, and every product attribute, since each attribute is a taxonomy of its own — so a translation WPML would refuse to link is never offered. WPML\'s own automatic translation bills per word in credits; the same words go through the Anthropic key already configured here for a fraction of that, in the shop\'s voice, with a glossary of terms that must never be translated. IT KEEPS ITS OWN REGISTER, field by field and language by language: one fingerprint of the source text as it stood when that field was translated. WPML re-marks a translation whenever the original is saved — and its signature covers the categories, the variation ids and custom fields, so renaming a category flags every product in it. The register answers the only question worth paying for: which WORDS moved. Nothing changed means nothing is sent, nothing is spent, and WPML is told the translation is up to date so the mark goes away. One field changed means that field alone is sent. A translation made elsewhere — through a spreadsheet, by hand — can be adopted instead: the source is recorded as it stands and the mark closed, without paying, and the screen says that is what happened. A PRODUCT IS MORE THAN ITS OWN FIVE FIELDS: the words each VARIATION carries travel with it and are written onto the variations WooCommerce Multilingual made (never onto one invented here — linking a variation is WCML\'s job). A translated variable product needs its attributes and its variations, and on a translation those are read-only — they can only come from a sync off the original — so every write asks WooCommerce Multilingual for that sync, the attributes first since they are the axes the variations are built along. That is plumbing and it is never narrated: no panel reports it and no button asks for it, and the ATTRIBUTE TERMS it is sold by are objects of their own, since one "Olive Drab" serves every product that carries it and is paid for once; the product\'s own panel lists the ones still missing a language and sends them to the same waiting list. Every edit screen of something WPML will link carries a "Translate with Dazont Ecom" button INSIDE WPML\'s own Language box — it opens the work, it never runs it: the panel on a product, and on everything else this screen with that one object already ticked. Price, stock, images and taxonomy structure are NOT touched: they belong to WooCommerce Multilingual, and when this module has to create a translation it asks WPML to run its own custom-field sync so the numbers arrive from the original rather than from us — a custom field WPML is set to COPY is never written here either, since the next sync would put the original\'s value straight back over the words. THERE IS ONE TRANSLATION SCREEN PER OBJECT, and it is WPML\'s own four steps: what is not translated or no longer up to date, the batch you send, the screen where you read every field beside its original and correct any word of it, and saving. It is reached from the batch list, from the waiting list, and from a "Translate with Dazont Ecom" button planted inside WPML\'s own Language box on the object\'s edit screen — never a popup or a panel of ours beside it. Nothing is written blind: a batch produces translations that WAIT, each one shown beside the original and beside what the translation holds today, and accepted field by field and language by language — a field you untick is left out, and a language left undecided keeps the object on the list. A translation this module did not write says so before it can be replaced. The prompt and the glossary live under Settings → Translation; WHICH FIELDS are sent is not a setting but a reading of WPML’s own rules, stated per kind of content on the dashboard — what is sent, what WPML copies and is therefore left alone, and what WPML wants translated that this module does not send, and every call is charged to the monthly budget like any other.', 'dazont-ecom' ),
			],
			'category_content' => [
			...
			'diagnostic' => [
				...
				'desc'  => __( 'Reads the shop against your own standards, lists what is missing, and hosts the work and the decisions (Dazont Ecom → Content).', 'dazont-ecom' ),
				'more'  => __( 'The question that comes before every other one: WHERE. A shop of a thousand products has no memory of which of them is short of a paragraph, which has one photograph in its gallery and which never got its second custom block, and a catalogue audited once in a spreadsheet is out of date the week after. This reads the shop and says so — and it is a to-do list, not a machine: nothing here writes, generates, spends or decides, and every line points at the screen that already fixes that one thing. The criteria are not this plugin\'s opinions: they are a LIST YOU EDIT, presented as the cards the prompt library uses — shut, a criterion is its name and its rule in words (\'Product · description is less than 120 words\'); open, it is the three controls that make it. A criterion is a FIELD, a COMPARISON and a figure — and nothing else unless you ask. A criterion is ONE rule and ONE figure by default, which is the whole of what it shows. A box marked Conditional opens a second, finer reading — \'If price is between 0 and 40 → at least 3 photographs\', as many conditions as you like, on EVERY criterion that holds a figure (a word count as much as a number of photographs; a rule that asks whether something is empty has nothing to put in tiers), each naming the field it measures so a price range and a stock range sit in the same list. Read top to bottom, the first that fits wins; anything none of them place is held to the figure above. Unticked on a fresh install and unticked for every criterion until somebody asks, so a shop that never opens it is held to one figure exactly as before; ticked, the conditions ARE the rule and the plain figure comes off the card — a figure beside them would be a second rule nobody wrote, and nothing could say which of the two a product had been held to. Anything no condition covers is not judged at all: nothing was asked of it. And the problem list is then GROUPED by condition — \'price 40 to 80 — at least 4\' over the products that condition placed — read from the same answer the reading used, so a heading can never disagree with the figure a product was held to. Unticking hides the conditions, stops them counting and gives the figure straight back, and it costs nothing — the field is one the nightly reading already loads. A cap at $16.90 and a plate carrier at $90 do not deserve the same gallery, and holding both to \'3\' is what makes a count on this screen a figure nobody believes. Ranges are half open, so 40 belongs to \'40 and 80\' and to nothing else. A product with nothing there to place it by — no price set — takes the plain figure, which is the strictest, so it shows on the list rather than being quietly excused by the cheapest condition. The criterion\'s NAME carries every figure, because a heading reading \'less than 6\' while a cheap product is judged on 3 is a heading that stopped being true. A criterion is a FIELD, a COMPARISON and a figure, in the vocabulary a product export is filtered with: is empty, is not empty, is less than, is at most, is more than, is at least, equals, does not equal, contains, does not contain — written as words or as symbols (&lt;, ≤, =), whichever you prefer, switched on the same screen. What can be read is deliberately wide: a product\'s title, description, short description, SEO title and description, SKU and any custom field; its main photograph AND that photograph\'s width, height and smallest side in pixels, so \'main photograph under 800px\' is a criterion and not a guess; its gallery, the links in its description, its price, sale price, stock, weight, categories, tags, attributes, variations, reviews, average rating and age in days; a category\'s description, internal links and how many products it holds; an article\'s links and length. A text is compared by its LENGTH — words for a description, characters for a title, because that is what each is actually short of — and by what it contains. Add your own, change a figure, restore the shipped list in one click. Nine ship, and they are rows like any other rather than code, so \'restore the default\' is putting those nine back and nothing else. Criteria saved before the comparisons existed keep meaning exactly what they meant. A criterion says WHICH POST TYPE it is about before it says anything else — Products, Product categories, Articles, Pages, and any custom type the site declares, read from WordPress rather than listed in this plugin — and everything below that follows from it: the field menu is cut to what that type actually has, and the custom-field box offers that type\'s own keys, read from the database, so the shop\'s branding blocks are in the list whatever wrote them. The scan then walks only the types some criterion asks about. There is no second source: what is on the Diagnostic is what is written in that list, in the order it was written. Criteria used to arrive from the prompt library as well — cleverly, and invisibly: they could not be found, edited or explained, and a screen that answers questions nobody asked is a screen nobody trusts. A block written by a prompt is checked the ordinary way, with an ordinary criterion (\'Product · custom field (text)\', key \'_bloc_text_2\', \'is empty\'), and the key box offers the fields this shop actually writes into, so it is picked from a list rather than remembered. Articles and pages are read in full — every published post and page, not the five hundred most recently touched, because the articles this screen is most often asked about are precisely the ones nobody has touched in a year. Thirteen things can be asked of one: its text, its title, its excerpt, its SEO title and description, any custom field, how long ago it was UPDATED and how long ago it was published, its featured image, the images and headings in the text, its comments, and its internal links — and the links are still held to the figure the linking pass works out, asked of the linking pass itself, so this screen and that one can never disagree about the same article. \'Article going stale — last updated more than 365 days ago\' ships switched on. A criterion you would rather not be counted against is unticked in the settings — nothing is deleted, it simply stops being called work. Each criterion\'s list of objects is its OWN database row, read only when that list is opened and never loaded with the rest — they all sat in one row before, read in full to draw fifty lines of one of them, which is why the list stopped at a thousand while the count said two thousand. The reading is done once a night in cron and kept: a screen that counted a thousand products on every load is a screen nobody opens twice, and the count on the menu is read from what was stored. "Read the shop again" does it on demand, once, with a lock so two clicks are one reading. Each line gives its count, what share of the shop that is, and the list behind it — where every entry also says what ELSE that product is short of, so a product needing four things is opened once instead of four times. That list is a working table, not a column of names: each product carries its price, how many UNITS it has sold, and the day its page was last edited — and any of the three sorts the whole list, not the fifty rows on screen, so \'the twenty best-sellers with a thin gallery\' is one click and not a spreadsheet. The figures are read in two queries for the entire list, whatever its length. There is no money column and there must not be one again: order lines are kept in the currency they were paid in, this shop\'s sales table holds hundreds of rows whose order no longer exists, and correct arithmetic on that input still printed $6,792,487 against 23 units. Units sold are what the screen shows — four sold is four sold in any currency. And the rows on screen are re-judged against the shop as it stands, not against the photograph the last reading took: a product you have just fixed is marked \'fixed ✓\' on the spot instead of sitting in the list until the next scan. And a line the plugin can MEND carries a button on every row — never a link to a settings page, which is what used to sit there twice. On a PRODUCT it opens the product\'s own popup: the same one the product screen and the products list open, on the section this criterion is about, with the work already laid out and nothing run. A product two photographs short of its gallery opens on two prompt rows, taken from the shop\'s own gallery prompts in the order they were written — a suggestion to look at, add a row to, change and only then generate, with the result in front of you before a word of it reaches the product. The block a criterion points at is read from a catalogue the product screen itself draws, keyed by what each block WRITES rather than by a prompt\'s name, so renaming a prompt breaks nothing and a criterion invented tomorrow arrives with its repair attached. A shop with no prompt aimed at that slot gets no button rather than one opening on an empty section. The same list can also be TICKED, a page at a time, and handed to the bulk screen the shop already generates from — the ids travel exactly as they do from the products list, and what to run is chosen there. On a category or an article the button runs that one pass and the result goes to the review list, where nothing reaches the shop until you accept it. A criterion the plugin cannot mend by itself offers no button at all. And all of it lives on ONE screen: reading what is wrong, doing something about it and saying yes or no to what comes back is a single piece of work, and it used to be three menu entries the owner had to connect himself — one of them reachable only through a redirect from a notice. It is now Dazont Ecom → Content, with WordPress\'s own tabs: the reading, and what waits for a decision. Each tab belongs to the module that does that work and appears only while it is on; a module switched off keeps its own page, so switching one off never takes a function with it. The menu badge is what waits for a PERSON — not the shortfall, which was a red "1,205" for ever on a menu you look at forty times a day, and a bubble you learn not to see. The shortfall is the figure on the tab that is about it. Only what falls short is listed, worst first, grouped by the screen the work is done on; the criteria that found nothing are named under it, folded, so one that quietly stopped matching is still visible. A criterion is NAMED BY WHAT IT DOES and there is no box to call it something else: \'Gallery photographs is less than 3 photographs\' follows the rule as the figure is changed, where a hand-written title stops being true the moment somebody edits the row and says nothing the rule did not already say. What the shop writes instead is the one thing the rule cannot know — a DESCRIPTION of what to do about it (\'Add more photographs to these products, to improve the conversion rate\'), shown under the name on the Diagnostic and beside it on the card. And every criterion says what it is FOR: SEO, CRO, both, or neither. That is not decoration — a shop is never working on being found and on converting in the same month, and a list holding both is a list read twice. The Diagnostic opens on three buttons carrying their own figure — Everything, SEO, CRO — each counting the THINGS waiting for that goal, once each however many criteria they fall short of, so \'CRO 4,831\' is products to open rather than boxes ticked. Choosing one is in the address, so the quarter that is about conversion is a bookmark and not a setting to keep in step; every line carries its goals as tags, so a number is always attributable without opening the criteria. Variations are read too, and by ATTRIBUTE: \'variations with no photograph of their own\' with attribute_pa_couleur in the key box counts, product by product, the colours that are still showing the parent\'s photograph — the exact gap the image lab fills one colour at a time. A variation set to \'any colour\' belongs to no colour in particular and is not counted as one; the key box offers the shop\'s real attributes, read from the variations themselves, so it is picked rather than remembered. It ships switched off, because which attribute matters is the shop\'s answer and not ours. One grouped query for the whole catalogue, so a shop with forty thousand variations pays for one query and not forty thousand.', 'dazont-ecom' ),
			],
			'mesh' => [
				'class' => 'DZE_Mesh',
				'group' => 'catalog',
				'label' => __( 'Internal linking', 'dazont-ecom' ),
				'desc'  => __( 'Reads who links to whom across the site, names the pages nobody points at, and says which pages should point at them (Dazont Ecom → Content → Linking).', 'dazont-ecom' ),
			...
			'automation' => [
				...
				'more'  => __( 'Adds no ability of its own: everything it launches is a function already on this site, with its own screen, its own prompt and its own button. What it brings is the part nobody can do by hand at scale — looking at the WHOLE site, seeing where each page falls short of what it should be, and working through the list a little at a time. It judges a page on the very figures that page\'s own panel prints: "1116 words, 0 links — target 750 words and 5 links" is read from the same place the panel reads it, so the screen and the robot can never disagree. Three tasks, each with its own switch, its own rhythm, and — where it writes to the shop — its own choice between saving straight away or waiting under "to review". Internal linking: ONE task for one piece of work, whatever kind of page either end of the link is, and it works in two phases in this order. FIRST the holes: the link graph — read across the whole site once a day — says which pages nobody points at and which pages should point at them, and the job carries those addresses so the pass writes THOSE links rather than whatever the page would have chosen on its own. THEN, once nothing is orphaned, the work done by hand until now: the pages carrying fewer links than their own length calls for, one per fifty words, each filling them from its own pool of related pages — which on a shop puts the product categories first. "Link the whole site" runs that second phase over every page at once, a few hundred per press, for a shop catching up from nothing; after that the daily pass is maintenance, picking up the pages you have just added. A page laid out by a page builder is a target and never a source, since its text is not in the post. Category descriptions: for a category with none, or one under two thirds of the length its branch deserves — held for review by default, and restricted to categories that have their SEMrush file unless you say otherwise. Marketing calendar: once a month, the commercial moments worth a promotion in the coming quarter, which land in the suggestion list where accepting one creates the event, disabled as always; moments already on the calendar are never proposed twice, and a pile of unanswered suggestions stops it asking for more. Every task shows what it is about to take — the next five pages and why each one is short — so the list it works from is on the screen rather than described. Work that writes text is handed to Content to review — one job at a time, in the background, under the monthly budget cap, and never on a copy of the shop unless a button is pressed by hand. The pace is the point: one item per pass, spread over its period, and never the same page twice within a month unless the pass changed nothing, in which case it comes back in three days. What was saved straight to the shop keeps the text it replaced: the last dozen passes are listed with what they changed, and one click puts any of them back. Adding a task later is adding a row to its list: a label, the module that owns the work, what counts as short, and how it is run.', 'dazont-ecom' ),
			],
			'reviews' => [
				'class'   => 'DZE_Reviews',
				'group'   => 'product',
			...
			'queue' => [
				...
				'desc'  => __( 'Sends batches off to be written, then holds everything generated for review (Dazont Ecom → Content to review).', 'dazont-ecom' ),
				'more'  => __( 'A description of two thousand words takes the model a minute or more, and a browser request that waits that long is cut off by the host — the HTTP 504 you would otherwise get. So nothing is written inside the request that asks for it: the selection is queued, a background worker takes one item at a time, and the screen only watches. Leave the page and it carries on (through Action Scheduler, which WooCommerce provides, or WP-Cron); stay on Dazont Ecom → Content → To review, the screen it draws, and the items go by one by one, as WPML does for translations. What comes back waits under "to review": open it, read it against what the category holds today, edit it in the WordPress editor, then accept or discard. Nothing reaches the shop until you accept, unless the batch was sent with immediate saving. Bulk actions on Products → Categories feed it. It carries photographs too, not only text: a product photograph waits here like everything else, reviewed as a picture — the new one beside the ones the product already shows, with Keep it, Make another, Throw it away. And it is the ONE screen that answers \"what is waiting for me\": the products holding a generated text or photograph nobody has decided on live on the bulk screen, which owns that work, and this names them and links straight to it — one menu, one count, rather than two of each that disagree. The bulk screen keeps its own address and every link ever printed at it, it simply stops being a second entry in the menu. And this screen lives under DAZONT ECOM rather than under Products: it holds categories, products and articles, and a list of everything the plugin has written does not belong inside one of the things it writes. Its old address redirects, so a bookmark still lands. Every decision on it is now SIGNED: the row says \"Accepted by Marie\" or \"Discarded by Paul\", read from what was stored when the button was pressed, and the products log names who decided each one — because a shop that hands this work to somebody else needs to see that a page was dealt with AND by whom. A pass that saved without review has nobody to name and says so rather than inventing one.', 'dazont-ecom' ),
			],
			'variation_split' => [
